    ...
    it('should transition from arrived to in_progress with picked_up_at', async () => {
      ...
      if (notifications && notifications.length > 0) {
        ...
        if (startNotif) {
          ...
          console.warn(
            '[Test A] Notification rows exist but none with type ride_started/pickup_started',
          );
        }
      } else {
        console.warn(
      ...
      if (res2.status === 200 || res2.status === 201) {
        ...
        if (notifications) {
          const startNotifs = notifications.filter(
            (n: any) =>
              n.type === 'ride_started' || n.type === 'pickup_started',
          );
        ...
        console.log(
          '[Test C] RPC returned 409 for already-started — acceptable behavior',
        );
      }
    });
  });
