...
const PENDING_TIMEOUT_MINUTES = parseInt(process.env.PAYMENT_PENDING_TIMEOUT_MINUTES || '15', 10);
...
export class PaymentsService {
  ...
  async createCheckout(
    ...
    const expiresAt = new Date(Date.now() + PENDING_TIMEOUT_MINUTES * 60 * 1000).toISOString();
  ...
  async processCallback(
    ...
    const { platformOrderId, providerPaymentId, status, signatureValid } = verification;

    // 3) Record the event (audit + idempotency)
    const eventKey = providerPaymentId || `${platformOrderId}-${payload.random_nr || Date.now()}`;

    const { error: eventError } = await this.paymentsRepository.insertProviderEvent({
    ...
    if (eventError?.code === '23505') {
      ...
      await this.paymentsRepository.insertAuditLog({
        ...
        entity_type: 'payment',
        entity_id: null,
        meta: { provider: providerName, event_key: eventKey, platform_order_id: platformOrderId },
      });
    ...
    if (!verification.ok || signatureValid === false) {
      this.logger.warn(`[processCallback] Signature invalid for ${platformOrderId}`);
    ...
    if (status === 'paid') {
      ...
      updates.failure_message = payload.error_message || 'provider_status_not_success';
      updates.failed_reason = payload.error_message || 'provider_status_not_success';
    }
    ...
    // 9) Analytics event
    track(status === 'paid' ? 'payment_succeeded' : 'payment_failed', {
      entityType: 'payment',
      entityId: payment.id,
    ...
    if (status === 'paid') {
      try {
        ...
        this.logger.error(`[processCallback] Exception during wallet credit: ${err}`);
  ...
  async expirePendingPayments(): Promise<number> {
    const { data: expired, error } = await this.paymentsRepository.findExpiredPendingPayments();
    ...
    this.logger.log(`[expirePendingPayments] Found ${expired.length} expired payments`);
   ...
   */
  private async creditCompanyWallet(
    paymentId: number,
  ): Promise<{
    ok: boolean;
    wallet_id?: number;
