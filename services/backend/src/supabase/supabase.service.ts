...
export class SupabaseService {
  ...
  /**
   * Create a per-request Supabase client which includes the user's access token
   * in the Authorization header so that auth.uid() and row-level-security
   * checks work inside Postgres RPCs.
   */
  getClientWithToken(accessToken: string): SupabaseClient {
    return this.anonClientWithAuth(accessToken);
  }
